...
impl View {
    ...
    /// Stable sequence number of the undo node the buffer currently shows.
    ///
    /// Unlike undo depth, this identity is not reused when history branches
    /// or the oldest retained nodes are pruned.
    pub fn current_undo_seq(&self) -> u64 {
        self.content.lock().unwrap().undo.current_node_seq()
    }
