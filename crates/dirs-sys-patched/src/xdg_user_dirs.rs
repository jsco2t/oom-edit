...
/// Local replacement for option_ext::OptionExt::contains.
/// Checks if an Option<T> contains a value matching the given reference.
fn option_contains<T, U>(opt: &Option<T>, x: &U) -> bool
where
    U: PartialEq<T>,
{
    match opt {
    ...
    for line in bytes.split(|b| *b == b'\n') {
        ...
        let key = if key.starts_with(b"XDG_") && key.ends_with(b"_DIR") {
            match str::from_utf8(&key[4..key.len()-4]) {
                Ok(key) =>
                    if option_contains(&user_dir, &key) {
