...
If the project root contains `.agent-context.local.md`, read it before doing
planning, architecture, requirements, or implementation work. Treat it as
additional project instructions.

## Markdown specification

The **Markdown Spec** located in the repo at `docs/markdown-spec.md` is the authoritative spec for the markdown parsing this project supports.
...
7. **Comment Clarity** Keep code comments clear and concise. Remove (or fix) dead code-comments when found. Avoid using terms which may prevent code understanding. Never directly reference a requirement number or bug number in a comment. These become stale. As an alternative, describe what the code is doing and that it's solving for a bug or a requirement.
...
4. `make check` passes.
5. `make lint` passes (clippy with `-D warnings`).
6. `make build` passes with **NO** `warnings` or `errors` (all build issues regardless of origin must be resolved)
7. **All** tests pass — `make test`, the entire suite, not just the new tests.
8. If dependencies changed: `make deny` and `make audit` pass and `vendor/` + `Cargo.lock` are updated in the same change.
...
Never include `Claude-Session:` lines, session URLs, or any Claude/AI attribution metadata in commit messages. Commit messages describe the change — they are not a place for tool provenance. Never include "co-authored by" comments.
